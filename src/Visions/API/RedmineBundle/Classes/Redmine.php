...
namespace Visions\API\RedmineBundle\Classes;
...
class Redmine {
    ...
    function __construct($config = array()) {
        ...
        $this->limit = (isset($config['limit'])) ? $config['limit'] : "";
        $this->offset = (isset($config['offset'])) ? $config['offset'] : "";        
    ...
    public function getUserId($username) {
        if (!is_array($this->users) or count($this->users) == 0)
            $this->listUsers();
    ...
    private function runRequest($restUrl, $method = 'GET', $data = "") {
        ...
        // Run the request
        try {
            curl_setopt($this->curl, CURLOPT_URL, $this->url . $restUrl . "?offset=" . $this->offset . "&limit=" . $this->limit);
    ...
    public function getProjects($projectId = null) {
        $url = "/projects";
        $url .= (isset($projectId)) ? "/" . $projectId : "";
        return $this->runRequest($url . '.xml', 'GET', '');
    }
    ...
    public function addIssue($subject, $description, $project_id, $category_id = 1, $assignmentUsernames = false, $due_date = false, $priority_id = 4) {
        ...
        if ($due_date)
            $xml->addChild('due_date', $due_date);
